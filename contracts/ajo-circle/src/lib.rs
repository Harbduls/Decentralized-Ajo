...
impl AjoCircle {
    ...
    pub fn deposit(env: Env, member: Address) -> Result<(), AjoError> {
        ...
        let round_contributions = members.iter().filter(|(_, m)| {
 ...
 main
        ...
        let mut cycle_withdrawals: Map<u32, Map<Address, bool>> = env.storage()
            .instance()
            .get(&DataKey::CycleWithdrawals)
            .unwrap_or(Map::new(&env));
        let mut cycle_map = cycle_withdrawals.get(cycle).unwrap_or(Map::new(&env));
    ...
    fn get_cycle_deadline(env: &Env, cycle: u32) -> Result<u64, AjoError> {
        let circle: CircleData = env.storage().instance().get(&DataKey::Circle).ok_or(AjoError::NotFound)?;
        let initial_deadline: u64 = env.storage().instance().get(&DataKey::RoundDeadline).unwrap_or(0);
        let cycles_elapsed = if cycle > 0 { cycle - 1 } else { 0 };
